...
class CategoryViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableViewOutlet.dequeueReusableCell(withIdentifier: "PrototypeT", for: indexPath) as! TableViewCell
        ...
        if let attack = pokemonArray![indexPath.row].attack {
            if let defense = pokemonArray![indexPath.row].defense {
                if let health = pokemonArray![indexPath.row].health {
                    cell.pokemonStat.text = String(describing: attack) + "/" + String(describing: defense) + "/" + String(describing: health)
                }
            }
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "CategorytoInfo", sender: Any?.self)
    }
